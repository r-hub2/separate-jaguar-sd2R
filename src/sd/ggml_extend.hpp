...
__STATIC_INLINE__ void print_sd_tensor(const sd::Tensor<T>& tensor, bool shape_only = false, const char* mark = "") {
    r_ggml_printf("%s: shape(", mark);
    for (size_t i = 0; i < static_cast<size_t>(tensor.dim()); ++i) {
        r_ggml_printf("%s%lld", i == 0 ? "" : ", ", static_cast<long long>(tensor.shape()[i]));
    }
    r_ggml_printf(")\n");
    r_ggml_fflush(NULL);
    if (shape_only) {
        ...
        for (int64_t i2 = 0; i2 < shape[2]; i2++) {
            ...
            for (int64_t i1 = 0; i1 < shape[1]; i1++) {
                ...
                for (int64_t i0 = 0; i0 < shape[0]; i0++) {
                    ...
                    r_ggml_printf("  [%lld, %lld, %lld, %lld] = ", static_cast<long long>(i3), static_cast<long long>(i2), static_cast<long long>(i1), static_cast<long long>(i0));
