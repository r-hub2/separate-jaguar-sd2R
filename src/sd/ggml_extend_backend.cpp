...
#include "sd2r_meta_backend.hpp"  // sd2R: meta backend (multi-GPU tensor split, behind a flag)
...
void SDBackendManager::reset() {
    ...
    runtime_assignment_ = {};
    params_assignment_  = {};
    // sd2R: free the meta backend (if built) alongside the regular ones
    sd2r::meta::MetaBackendOwner::instance().free();
}

ggml_backend_t SDBackendManager::runtime_backend(SDBackendModule module) {
    // sd2R "second path": multi-GPU tensor split via the meta backend (flagged).
    // Enabled only for DIFFUSION and only when the meta_backend flag is set.
    // If meta is unavailable / fails to build, fall through to the normal path.
    if (module == SDBackendModule::DIFFUSION && sd2r::meta::meta_enabled()) {
        ggml_backend_t meta = sd2r::meta::get_or_build_diffusion_meta_backend();
        if (meta != nullptr) {
            return meta;
        }
        // fallback: meta unavailable -> normal single-backend path (compatibility)
    }
