...
public:
    ...
    // sd2R low-level step-wise API (TODO 9.2): hooks to keep diffusion params
    // resident on the GPU across the per-step denoise calls of an R-driven
    // sampling loop. Currently NO-OP (correctness only); the batch-residency
    // optimization is a known debt — candidate implementation is the upstream
    // offload_params_to_runtime_backend / offload_params_to_params_backend.
    virtual void begin_batch_compute() {}
    virtual void end_batch_compute() {}
